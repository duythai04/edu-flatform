...
public class SubmissionController : ControllerBase
{
    ...
    [HttpPost]
    public async Task<IActionResult> Submit([FromForm] IFormFile? file,
                                            ...
                                            [FromForm] string? content)
    {
        ...
        if (fileUrl == null && string.IsNullOrWhiteSpace(content))
            return BadRequest("Vui lòng nộp file hoặc nhập nội dung bài làm.");

        var result = await _service.CreateAsync(
            assignmentId, studentId.Value, fileUrl, fileName, content);
    ...
    public async Task<IActionResult> Delete(Guid id)
    {
        ...
        // Chỉ chủ bài hoặc giáo viên mới xoá được
